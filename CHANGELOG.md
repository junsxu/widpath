...
### Changed
- Updated minimum Python requirement from 3.8 to 3.9 (README and documentation).
- `WidPathResolver.__init__` now declares an explicit `->None` return type annotation.
- Slice notation in `_split_wid` normalised to PEP 8 spacing (`wid[i : i + size]`).
- `WidPathResolver.__init__.py` now exports `__version__ = "1.0.0"`.

### Fixed
- CI badge URL in README pointed to `ci/yml` instead of `ci.yml`; corrected to a working link.
- Fix typo.
- Removed stray blank lines and trailing whitespace in source files and CI workflow YAML.
...
[Unreleased]: https://github.com/junsxu/widpath/compare/v1.0.0...HEAD
[1.0.0]: https://github.com/junsxu/widpath/compare/v0.2.0...v1.0.0
